...
- 답: Merge commit은 분기된 브랜치를 새로운 commit을 만들어 메인 브랜치로 통합시키는 방식, Squash and merge는 기존의 commit에 통합시키는 방식, Rebase and merge는 분기된 브랜치의 commit들을 메인 브랜치의 commit으로 만들어 통합시키는 방식이다.
...
- 답: 현재 작업 중인 commit임, commit message는 'docs: readme 추가'임
...
- 답: git revert <되돌리고 싶은 commit의 commit id>

### Q9
여러 사람이 협업하는 프로젝트에서 커밋을 되돌려야 한다.  
reset과 revert 중에 어떤 것을 선택할 것인지를 그 이유와 함께 적으시오.

- 답: 되돌린 자료에서 버그가 발생할 수 있고, 이에 대한 해결책이 예전 commit에 있을 수 있으므로 revert를 사용한다.